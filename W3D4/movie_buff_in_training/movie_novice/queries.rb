...
def below_average_years
  ...
  Movie.select('movies.yr, COUNT(title) as bad_movies').where("movies.score < 5")
    .group('movies.yr').order('movies.yr DESC')
end
...
def alphabetized_actors
  ...
  Actor.select('actors.*').order('actors.name').limit(10)
end
...
def pulp_fiction_actors
  ...
  Movie.select('actors.*').joins(:actors).where("movies.title = 'Pulp Fiction'")
end
...
def uma_movies
  ...
  Actor.select('movies.id, movies.title, movies.yr').joins(:movies)
    .where("actors.name = 'Uma Thurman'").order('movies.yr ASC')
end
